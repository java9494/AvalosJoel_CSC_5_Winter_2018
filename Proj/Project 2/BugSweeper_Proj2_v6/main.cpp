 ...
 * File:   main.cpp
 * Author: Joel Avalos
 * Created on February 7, 2018, 12:23 PM
...
void game(int [][COLS],bool [][COLS],char [][COLS],int [][COLS],int,char,int,int &,int,bool,bool);//Function to run the game
void cleared(int [][COLS],bool[][COLS],char[][COLS],int [][COLS],int,char,int,int &,int,bool &,bool &);//Function to clear each user-inputted space
...
int main(int argc, char** argv) {   
    ...
    nBugs=nBug(isitBug,ROWS);//Call nBug function to add up the total number of bugs in the game.
       
    //Start Menu
    start(title);//Call start menu function
    cin>>in;//Ask user for input
    switch (in){//This is the switch case for the menu
        case 1: game(spots,isitBug,clrDsp,nbrBugs,ROWS,row,spot,cleared,nBugs,gameLss,gameWin);break;//Run game
...
int nBug(bool bug[][COLS],int rows){//Adding up the number of bugs on the grid
    int nbugs=0;
    for (int x=0;x<rows;x++){
        for (int y=0;y<COLS;y++){
...
void nbrBugs(int nbrBugs[][COLS],bool bug[][COLS],int rows){//Testing neighboring
...
void game(int grid[][COLS],bool bug[][COLS],char spDsply[][COLS],int nbrBug[][COLS],//Game function
    ...
    cin>>row>>spot;
    cleared(grid,bug,spDsply,nbrBug,rows,row,spot,clrd,nbugs,lose,win);
    }while(lose!=true&&win!=true);
}

void cleared(int grid[][COLS],bool bug[][COLS],char spDsply[][COLS],int nbrBug[][COLS],//Clearing a space function
             int rows,char row,int spot,int &clrd,int nbugs,bool &lose,bool &win){
    ...
    if ((clrd+nbugs)==60){//The win condition of the game
