...
void start (string);
void game(int [][COLS],bool [][COLS],bool [][COLS],char [][COLS],int [][COLS],int,char,int,int,bool);
void cleared(int [][COLS],bool[][COLS],bool[][COLS],char[][COLS],int [][COLS],int,char,int,int &,bool &);
...
int main(int argc, char** argv) {
    ...
    cin>>in;
    switch (in){
        case 1: game(spots,isitBug,clrd,clrDsp,nbrBugs,ROWS,row,spot,cleared,gameLss);break;
...
void game(int grid[][COLS],bool bug[][COLS],bool clear[][COLS],char spDsply[][COLS],int nbrBug[][COLS],
          int rows,char row,int spot,int clrd,bool lose){
    do{
    display(grid,spDsply,rows);
    cout<<"Enter in a spot to clear."<<endl;
    cin>>row>>spot;
    cleared(grid,bug,clear,spDsply,nbrBug,rows,row,spot,clrd,lose);
    }while(lose!=true);
}
